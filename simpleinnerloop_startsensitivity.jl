...
ResultsHolder = zeros(numIncrementsCO2, nuIncrementsPM25)

for i = 1:numIncrementsCO2

    startCO2 = 100*i

    for j = 1:nuIncrementsPM25

        startPM25 = 100*j

        @variable(m, CO2[1:T+1] >= 0, start=startCO2)
        @variable(m, PM25[1:T+1] >= 0, start=startPM25)
        ...
        #print(m)
        solve(m)
        println(i, " ",j, " ", round(getobjectivevalue(m),1))
        ResultsHolder[i,j] = round(getobjectivevalue(m),1)

    end
end

println(ResultsHolder)
writecsv("output.csv", ResultsHolder)
